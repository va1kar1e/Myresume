<template>
  <v-col class="mb-1 py-2" cols="12" md="12" lg="12">
    ...
    <div class="title">
      <h2
        :class="`headline font-weight-light mb-4 text-uppercase ${item.color}--text`"
      >{{ item.univ }}</h2>
    ...
    <v-row>
      ...
      <v-col v-for="(item, i) in items" :key="i" cols="12" md="6" lg="6">
        <div class="team">
          <p :class="`font-weight-bold mb-4 text-left ${item.color}--text header-content`">Extracuricular</p>
          <p :class="`body-1 font-weight-light text-left black--text content`">{{ item.team }}</p>
          <p :class="`body-1 font-weight-light text-left black--text content`">{{ item.role }}</p>
        </div>
        <div class="archiev">
          <p
...
export default {
  ...
  data: () => ({
    ...
    item: {
      ...
      univ: "Kasersart University",
      deg: "Computer Engineering, B.Eng",
      role: "Student",
      project: [
        {
          name:
        ...
        {
          name:
            "Explore a danger area with a Robot using NodeMCU and P2P Camera",
          tag: ["Arduino", "ESP8266", "ExpressJS", "NodeJS", "Bootstrap"],
          video: "explerercar",
          images: ["explorercar/explorercar-page-0001.jpg"]
        }
      ]
    },
    items: [
